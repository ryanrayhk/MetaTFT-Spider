        ...
        if tab_name.lower() == 'players':
            ...
            if timeline_table:
                ...
                if headers:
                    ...
                    print(f"Found headers: {list(header_map.keys())}")
                
                # Get all rows except header
                rows = timeline_table.find_all('tr')[1:]  # Skip header row
                print(f"Found {len(rows)} timeline rows")
                
                for row in rows:
                    cells = row.find_all('td')
                    if len(cells) > 0:
                        # Get stage from first cell
                        stage = cells[0].get_text(strip=True)
                        # Get health value
                        health = cells[header_map.get('health', 7)].get_text(strip=True) if len(cells) > header_map.get('health', 7) else 'N/A'
                        print(f"Processing stage: {stage} {health}hp")
                        ...
                        stage_data = {
                            ...
                            'upgrades': cells[header_map.get('upgrades', 3)].get_text(strip=True) if len(cells) > header_map.get('upgrades', 3) else 'N/A',
                        ...
                        # Process board units if present
                        board_cell = cells[header_map.get('board', 1)]
                        if board_cell:
                            # Look for units in the board cell
                            unit_containers = board_cell.find_all('div', class_='StageUnitContainer')
                            print(f"Found {len(unit_containers)} units in board")
                            for container in unit_containers:
                                unit_img = container.find('img', class_='TableItemImg')
                                if unit_img:
                                    unit_data = {
                                        'name': unit_img.get('alt', ''),
                                        'tier': '1',  # Default tier since it's not shown in this view
                                        'items': []
                                    }
                                    
                                    # Get items from SmallItemsWrapper
                                    items_wrapper = container.find('div', class_='SmallItemsWrapper')
                                    if items_wrapper:
                                        items = items_wrapper.find_all('img')
                                        for item in items:
                                            if item.get('alt'):
                ...
                print(f"Processed {len(timeline_data)} stages")
                    ...
                    if isinstance(stage_data, dict):
                        ...
                        if 'units' in stage_data:
                            ...
                            for unit in stage_data['units']:
                                unit_text = f"- {unit['name']} (Tier {unit['tier']})"
                                print(unit_text)
                                clipboard_text.append(unit_text)
            else:
